...
public class CharacterAiAction extends MonoBehavior implements IAction {
	...
	public void Move(RELDIRECTION d) {
		FlatVector dir = (RELDIRECTION.toVector(d));
		characterMovement.setInputVector(dir);
		if (dir.y < 0)
			characterMovement.onJumpInput();
		characterAiMovement.setIsUse(false);
	}

	@Override
	public void Move(ABSDIRECTION d) {
		FlatVector dir = (ABSDIRECTION.toVector(d));
		characterMovement.setInputVector(dir);
		if (dir.y < 0)
			characterMovement.onJumpInput();
		characterAiMovement.setIsUse(false);
...
//		    vecList = Node.toVector(nodeList);
		...
		vecList.add(new FlatVector(cible.x, cible.y));
		characterAiMovement.resetFollowPoints();
