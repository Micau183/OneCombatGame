...
import src.Ronces.actions.RoncesAction;
import src.Ronces.actions.RoncesAttack;
import src.Ronces.actions.RoncesCondition;
import src.Ronces.actions.RoncesEntity;
...
public class Ronces extends GameObject{
	...
	public Ronces(float x, float y) throws IOException {
		...
		this.components.add(new RoncesEntity(this, this.transform, GameManager.instance.gameSettings.roncesAutomate));
		this.components.add(new RoncesAction(this, transform));
		this.components.add(new RoncesCondition(this, transform));
	}
}
